...
const { onGetterMessages: messages, onActionCreateImage } =
  STORE_APPLICATION.STORE_BOT_IMAGE.StoreBotImage();
...
const onSendMessage = async () => {
  ...
  const res = await onActionCreateImage({ prompt: inputMessageClone });
  ...
  <div class="relative flex flex-column h-full">
    ...
    <div class="w-full flex flex-column h-full">
      ...
      >
        ...
        <UserChat v-if="isAnswering" role="assistant" :content="[]" />
        ...
        >
          <div class="flex flex-column gap-3 align-items-center">
            <img
              ...
              src="/images/create-image-ai.jpg"
              alt="Lỗi ảnh"
            />
            <span class="text-center line-height-2">
              Chúng tôi có thể vẻ mọi thứ bạn mong muốn!
            </span>
          </div>
        </div>
      </div>
